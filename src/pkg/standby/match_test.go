...
func TestFloorAdmitsItsOwnTierAndEverythingStronger(t *testing.T) {
	...
	for _, floor := range legal {
		for _, cfg := range legal {
			...
			got, reason := Qualifies(approved(cfg), openLane(floor), T3, tm, now)
			if got != want {
...
func TestFloorAndItemTierBothGateConfiguration(t *testing.T) {
	tm := tiers(t)
	legal := []Tier{T1, T2, T3}
	for _, floor := range legal {
		for _, item := range legal {
			for _, cfg := range legal {
				want := cfg.strength() >= floor.strength() && cfg.strength() >= item.strength()
				got, reason := Qualifies(approved(cfg), openLane(floor), item, tm, now)
				if got != want {
					t.Errorf("floor %s, item %s, configuration %s: Qualifies = %v (%s), want %v", floor, item, cfg, got, reason, want)
				}
				wantReason := ReasonQualified
				if !want {
					wantReason = ReasonBelowFloor
				...
				if reason != wantReason {
					t.Errorf("floor %s, item %s, configuration %s: reason = %q, want %q", floor, item, cfg, reason, wantReason)
				}
			}
		}
	}
...
func TestUnknownItemTierNeverQualifiesAndSaysWhy(t *testing.T) {
	tm := tiers(t)
	for _, item := range []Tier{TierUnknown, Tier("unknown"), Tier(""), Tier("T0")} {
		ok, reason := Qualifies(approved(T1), openLane(T3), item, tm, now)
		if ok {
			t.Fatalf("item tier %q qualified; unknown items are not standby-eligible", item)
		}
		if reason != ReasonItemTierUnknown {
			t.Errorf("item tier %q: reason = %q, want %q — the explicit item guard states the invariant",
				item, reason, ReasonItemTierUnknown)
		}
	}
}
...
func TestUnknownConfigurationNeverQualifiesAndSaysWhy(t *testing.T) {
	...
	for _, floor := range []Tier{T1, T2, T3} {
		...
		ok, reason := Qualifies(approved(TierUnknown), openLane(floor), T3, tm, now)
		if ok {
	...
	ok, reason := Qualifies(Candidate{Contributor: "alice", Approved: true}, openLane(T3), T3, tm, now)
	...
	var empty TierMap
	if ok, reason := Qualifies(approved(T1), openLane(T3), T3, empty, now); ok || reason != ReasonConfigurationUnknown {
...
func TestWholeConfigurationIsCompared(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ok, reason := Qualifies(c, openLane(T1), T3, tm, now)
			if ok {
...
func TestApprovalAndSuspensionGateBeforeTheFloor(t *testing.T) {
	...
	c := approved(T1)
	c.Approved = false
	if ok, reason := Qualifies(c, openLane(T3), T3, tm, now); ok || reason != ReasonNotApproved {
	...
	c = approved(T1)
	c.Suspended = true
	if ok, reason := Qualifies(c, openLane(T3), T3, tm, now); ok || reason != ReasonSuspended {
...
func TestFloorThatIsNotATierFailsClosed(t *testing.T) {
	...
	for _, floor := range []Tier{TierUnknown, Tier("unknown"), Tier("T0"), Tier("any"), Tier("  ")} {
		ok, reason := Qualifies(approved(T1), LanePolicy{Floor: floor, DailyCap: 5}, T3, tm, now)
		if ok {
	...
	if ok, _ := Qualifies(approved(T1), LanePolicy{Floor: Tier("t1"), DailyCap: 5}, T3, tm, now); !ok {
...
func TestNobodyQualifiesIsANormalOutcome(t *testing.T) {
	...
	if got := QualifiedCount(pool, lane, T3, tm, now); got != 0 {
	...
	if got := QualifiedCount(pool, openLane(T2), T3, tm, now); got != 1 {
...
func TestQualifiedCountOverAMixedPool(t *testing.T) {
	...
	if got := QualifiedCount(pool, LanePolicy{Floor: T2, DailyCap: 2}, T3, tm, now); got != 2 {
		t.Errorf("QualifiedCount = %d, want 2", got)
	}
	if got := QualifiedCount(nil, LanePolicy{Floor: T2, DailyCap: 2}, T3, tm, now); got != 0 {
...
func TestRejectionCarriesNoDelta(t *testing.T) {
	all := []Reason{
		...
		ReasonSuspended, ReasonConfigurationUnknown, ReasonItemTierUnknown, ReasonBelowFloor,
		ReasonCapExhausted, ReasonFloorUnknown,
	}
	for _, r := range all {
	...
	tm := tiers(t)
	_, reason := Qualifies(approved(T3), openLane(T1), T3, tm, now)
