...
func TestDefaultCapDispatchesNothing(t *testing.T) {
	...
	for _, cap := range []int{0, -1, -50} {
		...
		ok, reason := Qualifies(c, lane, T3, tm, now)
		if ok {
...
func TestCapDecrementsAtDispatchAndResetsByExpiry(t *testing.T) {
	...
	if ok, reason := Qualifies(c, lane, T3, tm, now); !ok {
	...
	if ok, _ := Qualifies(c, lane, T3, tm, now); !ok {
	...
	ok, reason := Qualifies(c, lane, T3, tm, later)
	if ok {
	...
	if ok, _ := Qualifies(c, lane, T3, tm, atExpiry); !ok {
