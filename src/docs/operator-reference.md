...
| `agents.<lane>.standby.*` | `enabled: false`, `min_model_capability: T1`, `daily_cap_per_contributor: 0`. | v6 standby contributor configuration is validation-only at this stage. `enabled: true` requires `hub.standby_contributors`; floors must be `T1`, `T2`, or `T3`; `0` daily cap means no dispatch. |
| `hub.standby_contributors` / `hub.standby_model_tiers` / `hub.standby_allow_private_repos` | Empty approved list, empty model-tier map, private repos off. | Approval is a GitHub-login allow-list only, not a trust tier. Model tiers are owner-authored full reported configurations and ship empty so unmapped is unknown. Private repos require explicit opt-in because standby contributors receive task context. |
